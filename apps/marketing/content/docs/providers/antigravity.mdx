...
Synara does not read, refresh, write, or transmit Antigravity credential files. Authentication and
quota ownership remain inside the `agy` CLI.
...
Antigravity does not expose a standalone `agy usage` or `agy quota` command. Its documented
machine-readable usage surface is the status-line JSON hook. When Antigravity is enabled, Synara
adds its status-line receiver to a settings file only when no custom status-line command is already
configured. The receiver stores quota data in Synara's private state directory; it never accesses
Antigravity credentials or calls Antigravity's private backend endpoints.

Quota data is available after an Antigravity session emits a status-line update. If no session has
reported data, or the last update is older than fifteen minutes, Synara explains that the value is
unavailable or stale. Open an authenticated `agy` session and use `/usage` (also available as
`/quota`) to refresh Antigravity's own quota view.

If you already use a custom status-line command, Synara leaves it unchanged. In that case, keep
using the Antigravity `/usage` panel or manually compose a status-line command that forwards the
documented JSON payload to Synara.
...
- [Antigravity usage and quota](https://antigravity.google/docs/cli/commands/usage/)
- [Antigravity status line](https://antigravity.google/docs/cli/statusline)
