...
Synara connects to Oh My Pi through its `omp` executable and Agent Client Protocol mode. Oh My Pi owns the account, model routing, credentials, thinking levels, and provider service; Synara supplies the task, repository environment, transcript, approvals, review surfaces, and Git workflow around the session.
