 ...
 * deadlock — the request never settles, no `turn.completed` is ever emitted,
 * and the UI shows "Working" forever (observed in the wild as a turn stuck for
 * 15+ hours).
 *
...
export interface AcpTurnIdleWatchdogParams {
  /** How long the turn may go without real turn progress before it is force-failed. */
