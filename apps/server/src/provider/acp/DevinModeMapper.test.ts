...
describe("resolveDevinModeId", () => {
  ...
  it("full-access does not match code alias against a 'decode' mode id (token boundary)", () => {
    const modes = [makeMode("decode", "Decode"), makeMode("ask", "Ask")];
    const result = resolveDevinModeId({
      modes,
      runtimeMode: "full-access",
    });
...
describe("applyDevinModeSelection", () => {
  ...
  it.effect("does not restore to a 'task' mode via the ask alias (token boundary)", () =>
    Effect.gen(function* () {
      const setModes: string[] = [];
      const modes = [makeMode("task", "Task"), makeMode("plan", "Plan")];
      const modeState = makeModeState("plan", modes);
      const runtime = makeMockRuntime({
        modeState,
        onSetMode: (id) => {
          setModes.push(id);
      ...
      assert.deepStrictEqual(setModes, []);
    }),
  );
});
