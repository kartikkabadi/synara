...
function matchesTokenBoundary(haystack: string, alias: string): boolean {
  const haystackTokens = haystack.split(" ").filter(Boolean);
  const aliasTokens = alias.split(" ").filter(Boolean);
  if (aliasTokens.length === 0 || aliasTokens.length > haystackTokens.length) return false;
  for (let i = 0; i + aliasTokens.length <= haystackTokens.length; i++) {
    if (aliasTokens.every((token, j) => haystackTokens[i + j] === token)) return true;
  }
  return false;
}
...
function findDevinModeByAliases(
  ...
  // Token-boundary fallback on id + name only (exclude description to avoid
  // false positives). Whole-token matching prevents an alias like "ask" from
  // matching a mode id like "task", or "code" from matching "decode".
  return modes.find((mode) => {
    const haystack = normalizedModeText(`${mode.id} ${mode.name}`);
    return normalizedAliases.some((alias) => matchesTokenBoundary(haystack, alias));
  });
}
