...
describe("resolveDevinAcpAuthMethodId", () => {
  ...
  it.effect("selects browser_login when both methods are advertised without an API key", () =>
    Effect.gen(function* () {
      const method = yield* resolveDevinAcpAuthMethodId(
        {
          protocolVersion: 1,
  ...
  it.effect("rejects API-key-only authentication when WINDSURF_API_KEY is absent", () =>
    Effect.gen(function* () {
...
describe("DevinAdapterLive", () => {
  ...
  it.effect("composer capabilities advertise rollback as unsupported", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(capabilities.provider, "devin");
    }).pipe(
      Effect.provide(
        makeDevinAdapterLive({
          makeRuntime: () => Effect.succeed(makeMockRuntime()),
