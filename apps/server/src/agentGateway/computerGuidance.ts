...
export const COMPUTER_HELP_SECTIONS = {
  ...
  menus:
    "computer_list_apps lists installed/running apps and pids. computer_invoke_menu uses exact menu-bar titles; missing/disabled items refuse. It activates the app, so requires the user's visible-use request even in computer_run. computer_set_window_frame moves a window; observe unconfirmed results, never replay. computer_verify_state checks live predicates; computer_zoom magnifies window regions. computer_get_accessibility_tree lists apps/windows; contents stay computer_get_state. computer_get_cursor_position reads the pointer. computer_kill_app force-quits and loses unsaved work; prefer an authorized Quit.",
  hidden:
  ...
  foreground:
    'Foreground work is opt-in per task. computer_activate_window, computer_invoke_menu, delivery_mode:"foreground" and browser_prepare windowed:true require the user\'s visible-use request or a direct affirmative reply to a permission question such as "Can I bring the browser to the front?" Otherwise they refuse with foreground_not_requested. Naming an app or quoting an instruction is not authorization. On macOS launch_app opens without activation by default; hidden:true explicitly hides the app and may leave no usable window. If foreground input is essential, ask once and wait. foreground_user_interaction means wait for quiet. Never raise to bypass a background refusal or describe a window as shown unless the call succeeded.',
  forms:
...
export function computerToolInstructions(): string {
  return [
    ...
    "### Working loop",
    'Start with computer_launch_app or computer_list_windows({app}), then computer_get_state({window_id}). Act by ref (or exact label plus role); re-observe after navigation or layout changes. Use each action\'s observation; include_screenshot:false when elements suffice. set_value replaces a field; type_text with an exact window_id and writable ref uses semantic insertion and can overlap across different windows. Neither sends keydown/keyup; verify app reactions. press_key takes one key or a chord ("cmd+shift+n"); click takes count (1-3) and button.',
    "### Background first",
