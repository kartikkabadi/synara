...
import { ThreadLoop } from "@synara/contracts";
...
type ProjectionThreadLoopDbRow = typeof ProjectionThreadLoopDbRow.Type;
...
const makeProjectionThreadLoopRepository = Effect.gen(function* () {
  ...
  const deleteProjectionThreadLoopRow = SqlSchema.void({
    ...
    execute: ({ threadId }) =>
      sql`DELETE FROM projection_thread_loop WHERE thread_id = ${threadId}`,
  });
